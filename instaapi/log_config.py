...
====================================
Centralized logging setup for InstaAPI.
...
ROOT_LOGGER = "instaapi"


class LogConfig:
    """
    Centralized logging configuration for InstaAPI.

    One-call setup for all 15+ instaapi loggers.
    Supports console (colored) and file (rotating) handlers.

    Usage:
        LogConfig.configure(level="DEBUG", filename="instaapi.log")
    ...
    def get_logger(cls, name: str) -> logging.Logger:
        """
        Get a child logger under 'instaapi' namespace.

        Args:
            name: Logger name (e.g., 'client' → 'instaapi.client')
...
class DebugLogger:
    """
    Structured debug logger for InstaAPI.
    ...
    def __init__(self, enabled: bool = False, log_file: Optional[str] = None):
        ...
        self._logger = logging.getLogger("instaapi.debug")
        if enabled:
